...
/*
struct Cuboid: CustomStringConvertible {
    let p0: SIMD3<Int>
    let p1: SIMD3<Int>
    var value: Int
    ...
    var size: Int {
        let total = p1 &- p0 &+ SIMD3<Int>(repeating: 1)
    ...
    static func from(line: String) -> Cuboid {
        ...
        let onOffRange = Range(match.range(at: 1), in: line)!
        let value = line[onOffRange] == "on" ? 1 : -1
        ...
        let p0 = SIMD3<Int>(x: xMin, y: yMin, z: zMin)
        let p1 = SIMD3<Int>(x: xMax, y: yMax, z: zMax)
        
        return Cuboid(p0: p0, p1: p1, value: value)
    }
    
    func intersection(with other: Cuboid) -> Cuboid? {
        guard let xIntersection = lineIntersection(x0: p0.x, x1: p1.x, y0: other.p0.x, y1: other.p1.y) else { return nil }
        guard let yIntersection = lineIntersection(x0: p0.y, x1: p1.y, y0: other.p0.y, y1: other.p1.y) else { return nil }
        guard let zIntersection = lineIntersection(x0: p0.z, x1: p1.z, y0: other.p0.z, y1: other.p1.z) else { return nil }
        
        let p0 = SIMD3<Int>(x: xIntersection.lhs, y: yIntersection.lhs, z: zIntersection.lhs)
        let p1 = SIMD3<Int>(x: xIntersection.rhs, y: yIntersection.rhs, z: zIntersection.rhs)
        
        return Cuboid(p0: p0, p1: p1, value: 1)
    }
    
    private func lineIntersection(x0: Int, x1: Int, y0: Int, y1: Int) -> (lhs: Int, rhs: Int)? {
        let left = max(x0, y0)
        let right = min(x1, y1)
